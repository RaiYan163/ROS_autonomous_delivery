...
class JoystickTeleop(Node):
    def __init__(self) -> None:
        ...
        self._cmd_vel_type = str(self.get_parameter("cmd_vel_type").value).strip().lower()
        if self._cmd_vel_type not in ("twist", "twist_stamped"):
            self.get_logger().warn(
                "Invalid cmd_vel_type '%s'. Falling back to 'twist_stamped'."
                % self._cmd_vel_type
            )
            self._cmd_vel_type = "twist_stamped"
        ...
        self.get_logger().info(
            ...
            "linear_step=%.2f angular_step=%.2f cmd_vel_type=%s (edge-trigger)"
            % (
                self._axis_turn,
                self._axis_forward,
